...
def run_tests(test_folder_path: str, test_filter: str = None):
    ...
    base_current_directory = os.getcwd()

    result_path = os.path.join(
    base_current_directory, os.environ["TEST_RESULT_DIRECTORY"], "arcee_e2e", suite_name)
    ...
    print(f"Current working directory: {base_current_directory}")
    print(f"Absolute test path: {absolute_path}")
    ...
    current_directory = base_current_directory
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test-filter", required=False,
                        help="Filter to run specific tests.")
